  ...
  sections: [{
    ...
    children: [
      ...
      ...['Browser POSTs to /api/acc/connect with auth_type:"technical", client_id, client_secret, scope, instance_url',
          'Backend calls Adobe IMS POST /ims/token/v3 → gets access_token + expires_in (in seconds — do NOT divide by 1000)',
          'Saves encrypted_access_token, encrypted_credentials, token_expires_at to source_connections (no session_token or security_token)',
          'Creates user_sessions row (7-day rolling TTL)',
          'Sets acc_session cookie (httponly, 7 days)',
      ].map(bullet),
      ...spacer(),
      new Paragraph({ spacing: { after: 120 }, children: [
        new TextRun({ text: 'Modern bearer flow: ', bold: true }),
        new TextRun({ text: "All SOAP calls for technical auth send Authorization: Bearer <access_token> in the HTTP header with an empty sessiontoken in the SOAP envelope. The helper acc_soap_headers(conn, token) returns the correct headers per auth type — routes never branch on auth_type." })
      ]}),
      ...spacer(),
      p('Auto-refresh via get_valid_acc_token():', { bold: true }),
      code('If token_expires_at > now + 60s: return decrypt(encrypted_access_token)  (IMS token still valid)'),
      code('Else: decrypt(encrypted_credentials) → POST IMS /ims/token/v3 → new access_token'),
      code('      UPDATE source_connections SET encrypted_access_token, token_expires_at'),
      ...spacer(),
      ...
      p('Technical: token = decrypt(encrypted_access_token)  (IMS-refreshed if token_expires_at passed)'),
      p('Routes call acc_soap_headers(conn, token): technical returns Authorization: Bearer, classic returns Cookie + X-Security-Token. No auth-type branching needed in SOAP envelope builders.'),
      pageBreak(),
      ...
      p('Always uses live ACC SOAP. A DB-only approach was considered but rejected — users extract only a subset of schemas, so the DB would miss dependencies for schemas never selected for extraction, silently showing dependent schemas as selectable.'),
      ...spacer(),
      table(
        ['Step', 'Action', 'Details'],
        [
          ['1', 'Authenticate', 'get_login_from_cookie() → login_id. SELECT source_connections WHERE login_id = ? → get_valid_acc_token(conn, db)'],
          ...
          ['4', 'Build graph', 'For each link where targetSchema namespace is not a system namespace: dependents_of[target].append(schema_key), dependent_set.add(schema_key)'],
          ['5', 'Return', '{ dependents_of: {...}, dependent_set: [...] } — not stored in DB, computed fresh on every page load'],
        ],
        [600, 2400, 6360]
      ),
      ...spacer(),
      ...
      p('Auth: acc_session cookie → login_id via get_login_from_cookie(). No body. No DB write.'),
      p('ACC SOAP calls made:'),
